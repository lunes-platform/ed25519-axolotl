...
fn sign_message(secret_key: &Vec<u32>, msg: &Vec<u32>, opt_random: &Vec<u32>) -> Vec<u32> {
    if opt_random.len() > 0 {
...
fn open_message(public_key: &Vec<u32>, signed_msg: &mut Vec<u32>) -> Vec<u32> {
...
fn sign(secret_key: &Vec<u32>, msg: &Vec<u32>, opt_random: &Vec<u32>) -> Vec<u32> {
    let mut len = 64;
    if opt_random.len() > 0 {
        len = 128;
    }
...
fn verify(public_key: &Vec<u32>, msg: &Vec<u32>, signature: &Vec<u32>) -> isize {
...
#[allow(warnings)]
fn random_bytes(size: usize) -> Vec<u32> {
    let High: u32 = 255;
    let Low: u32 = 0;
...
// 	let mut sigmsg = signMessage(&keys.private_key, &msg, &rnd);
// 	let msg2 = openMessage(&keys.public_key, &mut sigmsg);
